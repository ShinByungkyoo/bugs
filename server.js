...
// Enable CORS and JSON parsing (increase limit for base64 image uploads)
app.use(cors());
...
// API endpoint for pest identification
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({ 
        error: "API 키가 등록되어 있지 않습니다. 설정에서 키를 등록하거나 서버 설정을 확인해 주세요." 
      });
    }
    ...
    // Perform server-to-server request to Gemini API
    ...
    console.error("서버 해충 식별 중 에러 발생:", error);
...
// Handle single-page app routing in production
app.get(/.*/, (req, res) => {
